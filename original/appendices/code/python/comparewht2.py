...
# india numbers
startedtime = 0.0080183976280671 # from plot_startup.py
xfs = [load(file) for file in sorted(glob('/media/qungibbo/data/sims/mpia/india/scripts/npydata/xf.*.npy'))]
whts= [load(file) for file in sorted(glob('/media/qungibbo/data/sims/mpia/india/scripts/npydata/wht.*.npy'))]
print(len(xfs))
rawtimes = scrape_attrs('/media/qungibbo/data/sims/mpia/india/slices.h5', 'results/cells/list_1', 'time')
rawtimes = array(list(map(float,rawtimes)))
rawtimes = sort(rawtimes)
rawtimes = rawtimes[:len(whts)]
dindia = get_wht(xfs, whts, rawtimes, startedtime)
...
thisimage = []
thisimage.extend(axes.plot(dindia['xf'],dindia['mwht'], 'b-',linewidth=1))
thisimage.extend(axes.plot(dgolf['xf'],dgolf['mwht'], 'g-',linewidth=1))
#thisimage.extend(axes.plot(dhotel['xf'],dhotel['lwht'], 'r--',linewidth=1))
#thisimage.extend(axes.plot(dhotel['xf'],dhotel['uwht'], 'r--',linewidth=1))
images.append(thisimage)

fill = plt.fill_between(dindia['xf'],dindia['lwht'], dindia['uwht'], alpha=0.2, facecolor='blue')
fill = plt.fill_between(dgolf['xf'],dgolf['lwht'], dgolf['uwht'], alpha=0.2, facecolor='green')
plt.legend([images[0][0], images[0][1]], labels=['Medium Grid HT','Fine Grid HT'],loc='upper left')
