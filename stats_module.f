...
c *****************************************************
      ...
      real*8, dimension(:,:), allocatable :: sm,sp,xcn
      ...
      ! -- Read mean and stats
      call h5ex_att_get(pob%rid,'stats_time',stat_time,ier) !?build
...
! --------------------------------------------------------------
      ...
      function exv_stats_(pob,np,ige,s,nexv,exv_names) result (stats)
      implicit none
      type(pob_t), intent(IN), target :: pob
      integer, intent(IN) :: np,nexv
      integer, dimension(np), intent(IN) :: ige
