...
! *****************************************************
! I made this with the best of intentions, but it segfaults????
! *****************************************************
...
!***********************************************************************
! **** Compute the turbulent timescales needed for TCI formulae
!      
!      Written by:    Nick Gibbons (CfH, 15.11.03 n.gibbons@uq.edu.au)
!      Modified  :    
!***********************************************************************
...
!***********************************************************************
! **** Compute the LES filter width based on gehre_phd's method
!      
!      Written by:    Nick Gibbons (CfH, 15.11.03 n.gibbons@uq.edu.au)
!     Modified  :    
!***********************************************************************
...
! *****************************************************
!  Subroutine to co-ordinate IO for stats data
!  @curator: Nick Gibbons
!  Notes: I inherited this from Rolf, but I beleive it
!  to have originateed from someone at UoM
! *****************************************************
...
!
      ...
      Case('w')
         call us3d_wlog('== Writing stats HDF5 solution to: ' &
                        //trim(solf), l=LOG_STATE,i=0,aei=0,e=olun)
         call stats_write(solf,inum,ier)
         if (ier/=0) goto 999

      Case('r')
         call us3d_wlog('== Reading stats HDF5 solution from: ' & 
                       //trim(solf),l=LOG_STATE,i=0,aei=0,e=olun)
      ...
      if (us3d_cklog(i=0,l=LOG_TIMING)) then
         write(loglun,fmt='(1x,a,g15.5,1x,a)') &
                          '-- Data IO took ',rtime,'seconds'
      endif
...
! *****************************************************
!     Routine to read the stats solution file in HDF5
!     format.  Note, only interior cells are read.
!
!     Input
!     -----
!     fname   - The HDF5 filename to read
!     inum    - (Optional) solution number.  Defaults to 1.
!
!     Output
!     ------
!     ier     - Returns nonzero on an error
! *****************************************************
...
!     *** Each node stores their cell mapping
...
!     *** Set the solution number integer and string
      if (us3d_debug.and.id==0) &
         write(olun,*) "-- Will read solutionfrom run: ",inum

!     *** Initializes file and groups
      new= .false.
      if (us3d_debug.and.id==0) write(olun,*) '-- Opening solution file'
      call us3d_sfile_init(fname,new,sfile,ier)
      if (ier/=0) goto 999

!     *** Open or create group for this run
...
!     *** Read solution data globally according to the global map
      ...
      if (us3d_debug.and.id==0) &
          write(olun,*) 'Reading solution variables in parallel'
      ...
      if (us3d_debug.and.id==0) &
         write(olun,*) '-- Finished reading stats'
!
!     *** Check for any errors in above section
...
!     *** Cleanup
      ...
      return
 901  if (id==0) write(olun,*) &
                 '*** Error allocating memory in subroutine stats_read'
...
!c        *** Decide whether to create a new solution file
         ...
         if (us3d_debug) write(olun,*) &
                '-- Initializing solution file: "'//trim(fname)//'"'
         call us3d_sfile_init(fname,new,sfile,ier)
         if (ier/=0) goto 201
...
!c        *** Attributes stored for this run
 ...
 201  if (hdf_err<0) ier= 1
      ...
      if (us3d_debug.and.id==0) &
         write(olun,*) '-- Finished writing stats'
...
!c        *** Close file
      ...
      return
 901  if (id==0) & 
         write(olun,*) '*** Error alloc. mem. in subroutine stats_write'
